    ...
    <div class="card-body">
        <form action="{{ route('locker.update', $locker->id) }}" method="post">
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control @error('locker_name') is_invalid @enderror" value="{{ isset($locker) ? $locker->locker_name : old('locker_name')}}" placeholder="Enter your code" name="locker_name" required>
                @error('locker_name')
            ...
            <div class="mb-3">
                ...
                <select name="major_name" class="form-select" required>
                    ...
                    <option value="Web Programming" {{ $locker->major_name == 'Web Programming' ? 'selected' : '' }}>Web Programming</option>
                    <option value="Content Creator" {{ $locker->major_name == 'Content Creator' ? 'selected' : '' }}>Content Creator</option>
                    <option value="Teknisi Jaringan" {{ $locker->major_name == 'Teknisi Jaringan' ? 'selected' : '' }}>Teknisi Jaringan</option>
                </select>
            </div>
              <div class="mb-3">
                ...
                <select name="status" class="form-select" required>
                    ...
                    <option value="Unavailable" {{ $locker->status == 'Unavailable' ? 'selected' : '' }}>Unavailable</option>
